...
function MachineLib:__init()
	...
	self.lolPatch = GetGameVersion and GetGameVersion():sub(1,4) == "6.20" and 1 or 2
	self.notPresentPatch = GetGameVersion and GetGameVersion():sub(1,4) ~= "6.19" and GetGameVersion():sub(1,4) ~= "6.20"
	self.cn = self.notPresentPatch
	
	--Send--
		self.moveHeader = self.lolPatch == 1 and 16 or 120
		self.spellHeader = self.lolPatch == 1 and 246 or 272
		
		--cspell2
		self.cspell2Header = self.lolPatch == 1 and 363 or 377
		self.cspell2Pos = self.lolPatch == 1 and 11 or 23
		self.cspell2Byte = self.lolPatch == 1 and 0x40 or 0x40
	
	--Rsv--
		self.goldHeader = self.lolPatch == 1 and 166 or 341
		
		--Buy
		self.bHeader = self.lolPatch == 1 and 369 or 187
		self.bPos = self.lolPatch == 1 and 13 or 13
		self.botrk = self.lolPatch == 1 and 98 or 250
		self.IE = self.lolPatch == 1 and 202 or 183
		self.Trinity = self.lolPatch == 1 and 249 or 84
	
		--Recall
		self.recallHeader = self.lolPatch == 1 and 101 or 259
		self.recallPos1 = self.lolPatch == 1 and 31 or 80 --id
